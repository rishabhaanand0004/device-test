...
// ---------------- [FIXED] Mutex for thread-safe active cards access ----------------
// Protect ALL g_activeCards / g_activeSet operations (reads AND writes) with portMUX.
// This prevents race conditions between Firebase updates and RFID scanning in loop().
...
portMUX_TYPE g_fileMux = portMUX_INITIALIZER_UNLOCKED;
// ---------------------------------------------------------------------------------

// ---------------- [FIXED] Non-blocking relay control ----------------
...
// ---------------- [FIXED] RFID debouncing state tracking ----------------
// Prevent double-reads and ensure clean card removal detection
...
// ---------------- [FIXED] Non-blocking time sync state ----------------
...
// ---------------- [FIXED] Memory monitoring and watchdog ----------------
...
void checkHeapHealth() {
  ...
  if (freeHeap < MIN_FREE_HEAP_BYTES) {
    Serial.printf("CRITICAL: Heap exhausted! free=%u, forcing emergency flush\n", freeHeap);
    ...
    if (ESP.getFreeHeap() < MIN_FREE_HEAP_BYTES) {
      Serial.println("CRITICAL: Heap still low after flush, restarting...");
      delay(100);
      ESP.restart();
    }
  } else if (freeHeap < HEAP_WARNING_THRESHOLD) {
    Serial.printf("WARNING: Low heap detected: free=%u\n", freeHeap);
  }
}

// Reset watchdog timer to prevent device lockup
void feedWatchdog() {
  esp_task_wdt_reset();
}

// [FIXED] Count lines in log queue file to prevent unbounded growth
...
// [FIXED] Optimized UID normalization with better memory handling
...
// [FIXED] Thread-safe file append with mutex protection
...
// [FIXED] RFC3339 timestamp with validation
String rfc3339NowUTC() {
  time_t now = time(nullptr);
  // Validate time is set (after year 2020)
  if (now < 1577836800) {
    Serial.println("WARNING: System time not set, using epoch");
...
// [FIXED] Optimized JSON escaping with better memory pre-allocation
String escapeJson(const String &s) {
  String out;
...
// [FIXED] Current time with validation
String currentHHMM() {
  time_t now = time(nullptr);
...
// [FIXED] Load config with input validation
void loadConfigIfPresent() {
  ...
  // [FIXED] Validate config file is not empty or corrupted
  if (cfg.length() == 0 || cfg.indexOf("{") < 0) {
    Serial.println("WARNING: Config file empty or corrupted, treating as missing");
    g_ssid = g_pass = g_apiKey = g_projectId = g_userEmail = g_userPass = g_businessId = g_deviceName = "";
    return;
  }
  ...
  // [FIXED] Log warning if critical fields are missing
  if (g_ssid.length() == 0) Serial.println("WARNING: wifi_ssid is empty in config");
  if (g_apiKey.length() == 0) Serial.println("WARNING: api_key is empty in config");
  if (g_projectId.length() == 0) Serial.println("WARNING: project_id is empty in config");
}
...
// [FIXED] Non-blocking time sync - call from setup() to start, then poll from loop()
void timeSyncStart() {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("timeSyncStart(): WiFi not connected; skipping NTP sync.");
    return;
  }
  ...
  Serial.println("Starting non-blocking time sync (SNTP)...");
...
// [FIXED] Poll time sync status - call from loop() after timeSyncStart()
void timeSyncPoll() {
  if (g_timeSyncState == TIME_SYNC_IDLE) return;
  if (g_timeIsSynced) return; // already synced
  
  unsigned long now = millis();
  
  if (g_timeSyncState == TIME_SYNC_SNTP_WAIT) {
    time_t t = time(nullptr);
    if (t > 1577836800) {
      // SNTP succeeded
      Serial.println("Time sync: SNTP OK");
    ...
    // Check timeout
    if (now - g_timeSyncStartMs > TIME_SYNC_SNTP_TIMEOUT_MS) {
      Serial.println("Time sync: SNTP timeout, will continue without time sync");
      // Don't block operation - device can work without accurate time
      g_timeSyncState = TIME_SYNC_IDLE;
      // Note: Not setting g_timeIsSynced = true, so timestamps will use epoch
    }
  }
}
...
void timeSync() {
  Serial.println("WARNING: timeSync() is deprecated, use timeSyncStart() + timeSyncPoll()");
  timeSyncStart();
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (t > 1577836800) {
      g_timeIsSynced = true;
      Serial.println("Time synced (blocking mode)");
      return;
    }
  }
  Serial.println("Time sync incomplete (blocking mode timed out)");
}
...
void otaProgressCallback(size_t progress, size_t total) {
  ...
  if (now - lastPrint > 2000 || progress == total) {
    ...
    // Feed watchdog during OTA to prevent timeout
    feedWatchdog();
  }
}
...
bool performOtaUpdate(const String &url) {
  ...
  // Use insecure connection for simplicity (you can add certificate validation)
  WiFiClient client;
...
void checkForOtaUpdates() {
  ...
  unsigned long now = millis();
  if (now - g_lastOtaCheck < OTA_CHECK_INTERVAL_MS) return;
  
  g_lastOtaCheck = now;
  
  Serial.println("OTA: Checking for firmware updates...");
  
  // Query Firestore for firmware update info
  // Document path: firmware_updates/latest
  String docPath = "firmware_updates/latest";
  String payload = Docs.get(aClient, Firestore::Parent(g_projectId), docPath, GetDocumentOptions());
  
  if (aClient.lastError().code() != 0) {
    Serial.printf("OTA: Failed to check updates: %s\n", aClient.lastError().message().c_str());
    return;
  }
  ...
  if (remoteVersion.length() == 0 || downloadUrl.length() == 0) {
    Serial.println("OTA: No update information found");
    return;
  }
  ...
  if (remoteVersion != currentVersion) {
    Serial.printf("OTA: Update available! Version %s\n", remoteVersion.c_str());
    ...
    if (mandatory) {
      Serial.println("OTA: Mandatory update - starting immediately");
...
void fetchActiveCardsFromFirebase_v2() {
  ...
  // 5) [FIXED] Replace in-memory list atomically with proper critical section
  portENTER_CRITICAL(&g_activeCardsMux);
...
// ---------------- [FIXED] Thread-safe helper: findActiveCardTimes with mutex protection ----------------
...
// ---------------- [FIXED] Non-blocking relay control functions ----------------
...
void asyncCB(AsyncResult &r) {
  ...
  if (r.isEvent()) {
    ...
    if (lmsg.indexOf("400") >= 0 || lmsg.indexOf("bad request") >= 0 || lmsg.indexOf("permission_denied") >= 0) {
      ...
      // [FIXED] Use named constants for backoff calculation
...
void firebaseInitPoll() {
  ...
  if (now - g_firebaseInitStartedAt >= FIREBASE_INIT_POLL_TIMEOUT_MS) {
    ...
    // [FIXED] Backoff next attempt to allow TCP/IP/wifi to settle
...
// [FIXED] Device registration with error logging
bool fsRegisterOrUpdateDevice() {
  bool ok1 = fsUpsertRootDevice();
  if (!ok1) {
    Serial.println("WARNING: fsUpsertRootDevice failed");
  }
  
  bool ok2 = (g_businessId.length()==0) ? true : fsUpsertBusinessDevice();
  if (!ok2 && g_businessId.length() > 0) {
    Serial.println("WARNING: fsUpsertBusinessDevice failed");
  }
...
// [FIXED] Load access list with proper mutex protection
void loadAccessListFromFile() {
  ...
  // [FIXED] Protect file read with mutex
  portENTER_CRITICAL(&g_fileMux);
  ...
  // [FIXED] Atomic update of active cards with proper critical section
  portENTER_CRITICAL(&g_activeCardsMux);
  g_updatingActiveCards = true;
  g_activeCards.clear();
  g_activeSet.clear();
...
// [FIXED] Queue access log with memory optimization and error handling
...
void flushQueueIfNeeded() {
  ...
  // [FIXED] Also flush if queue has too many lines (check less frequently to avoid overhead)
...
void relayGrantPulse() {
  Serial.println("WARNING: relayGrantPulse() is deprecated, use relayStartPulse()");
  relayStartPulse();
}
...
// [FIXED] Read UID with debouncing and state tracking
// Returns empty string if no new card or if same card scanned too recently
String readUidHexFrom(MFRC522 &r, RfidReaderState &state) {
  ...
  // [FIXED] Build UID with validation
  char buf[8];
  String out;
  ...
  // [FIXED] Debounce: ignore if same card scanned too recently
...
// [FIXED] Improved WiFi recovery with better state management
void wifiRecoveryTick() {
  unsigned long now = millis();
  ...
  if (elapsed < WIFI_LOOKUP_TIMEOUT_MS) {
    ...
    // [FIXED] Use named constant for log interval
...
void setup() {
  ...
  // [FIXED] Initialize watchdog timer early to catch boot hangs (ESP-IDF v5.x API)
  ...
  Serial.printf("Watchdog initialized: %d second timeout\n", WATCHDOG_TIMEOUT_SEC);
  ...
  // [FIXED] Configure hardware pins early with validation
  ...
  Serial.println("Hardware pins configured: relay locked");
  ...
  // [FIXED] RFID bring-up with error checking
  SPI.begin();
  ...
  Serial.printf("MFRC522 Entry version: 0x%02X\n", entryVer);
  Serial.printf("MFRC522 Exit version: 0x%02X\n", exitVer);
  
  if (entryVer == 0x00 || entryVer == 0xFF) {
    Serial.println("WARNING: Entry RFID reader not responding properly");
  }
  if (exitVer == 0x00 || exitVer == 0xFF) {
    Serial.println("WARNING: Exit RFID reader not responding properly");
  }
  
  Serial.println("MFRC522 (entry & exit) initialized. Ready to scan cards.");
  feedWatchdog();
  ...
  if (!configComplete()) {
    Serial.println("Config incomplete or missing - starting provisioning portal");
    // no config.json or incomplete → start setup AP (normal window)
    startProvisionPortal(0); // 0 = normal (non-forced) window
  } else {
    Serial.println("Config loaded - attempting WiFi connection");
    // try Wi-Fi first
    if (!wifiConnect()) {
      ...
      feedWatchdog();
      // [FIXED] got IP → start non-blocking time sync & init Firebase
      timeSyncStart();
      feedWatchdog();
      firebaseInitStart();
      ...
      if (!app.ready()) {
        ...
        Serial.println("Firebase ready - entering normal operation mode");
        // Firebase ready → normal operation
        bool regOk = fsRegisterOrUpdateDevice();
        if (!regOk) {
          Serial.println("WARNING: Device registration failed, will retry on heartbeat");
        }
...
void loop() {
  // [FIXED] Feed watchdog at start of every loop iteration
  feedWatchdog();
  
  // [FIXED] Check heap health periodically
  ...
  // [FIXED] Update non-blocking relay state machine
  relayUpdate();
  
  // [FIXED] Poll time sync if in progress
  timeSyncPoll();
  ...
  String uid = readUidHexFrom(rfidEntry, g_entryReaderState);
  if (uid.length()) {
    bool grant = false;
    ...
    // [FIXED] Protect set access with mutex
    portENTER_CRITICAL(&g_activeCardsMux);
    ...
    // [FIXED] Use non-blocking relay pulse
    if (grant && g_activityEnabled) relayStartPulse();
  ...
  // ---------- RFID EXIT ----------
  // [FIXED] Use debounced reader with state tracking
  String uid2 = readUidHexFrom(rfidExit, g_exitReaderState);
  if (uid2.length()) {
    bool grant = false;
